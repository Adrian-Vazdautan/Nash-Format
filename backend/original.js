...
function disable_NEW_dark_theme() {
    const linkElement = document.getElementById('NIGHT_MODE');
    let href = linkElement.getAttribute('href');
    // Проверяем, что в значении атрибута href нет символа #
    if (href && !href.includes('#')) {
        // Добавляем символ # только если его нет
        href = '#' + href;
...
function enable_NEW_dark_theme() {
    const linkElement = document.getElementById('NIGHT_MODE');
    const href = linkElement.getAttribute('href');
    // Проверяем, что в значении атрибута href есть символ # и он находится в начале строки
    if (href && href.startsWith('#')) {
        linkElement.setAttribute('href', href.slice(1));
    }
}
...
{//	MENU || Categoryes
	...
	//Function which will: hide MENU
		function fwwsm(){
			...
			/*Hide menu*/document.getElementsByClassName("c1js")[0].classList.add("mobile_none");
			/*Enable : <Body> scroll->*/document.getElementsByClassName("overflowjs")[0].classList.remove("ovysi_2");
		}
	//Function which will: show MENU
		function fwwhm(){
			...
			/*Show menu*/document.getElementsByClassName("c1js")[0].classList.remove("mobile_none");
		}
	//Active function fwwhm() which will hide MENU when making click outside menu or button
		document.addEventListener('click', function(event){
			let button = event.target.closest(".menujs"),
				menu   = event.target.closest(".APPEARANCE_spwh_js");
			if(button || menu){
				return;
			}
			/*Hide menu*/fwwsm();
...
function MenuAuto(){//	Menu auto
	...
	//START : APPEARANCE
		...
		//Appearance
			//Light Mode
				if(localStorage.getItem("appearance") == "light_mode"){
					menu_image_address = "url(../../../../../../src/du/icon/menu/light/";
				}
			//Dark Mode
				else if(localStorage.getItem("appearance") == "dark_mode"){
					menu_image_address = "url(../../../../../../src/du/icon/menu/dark/";
				}
	//END
...
{//	hide-post-complain.js
	...
	/*START*/
		function complain(nr){
			if(localStorage.getItem("session") == "true"){
					...
					document.getElementsByClassName("coar0js")[0].classList.remove("none");
					document.getElementsByClassName("overflowjs")[0].style.overflowY = "hidden";
	...
	function ccancel_0(){
		document.getElementsByClassName("coar0js")[0].classList.add("none");
		document.getElementsByClassName("overflowjs")[0].style.overflowY = "scroll";
	...
	if(localStorage.getItem("session") == "true"){
		...
		thnkBell.onclick = function(){
			...
			function hide_notifyes(){
				...
				if(document.getElementsByClassName("tBm_count_js")[0]){
	...
	function show_aua_0(){
		//When open
			if(avatarBgColor == 1){
				...
				//Enabled version
					//mobile
						if(localStorage.getItem("enabled_version") == "mobile"){
							/*Disable : <Body> scroll->*/document.getElementsByClassName("overflowjs")[0].classList.add("mobile_hide_scroll_Y_css");
						}
					//desktop
						else if(localStorage.getItem("enabled_version") == "desktop"){
							/*Enable : <Body> scroll->*/document.getElementsByClassName("overflowjs")[0].classList.remove("mobile_hide_scroll_Y_css");
						}
			}
		//When closed
				...
				avatarPopUpMenu.classList.add("none");
		  		...
		  		/*Remove selected focus from button*/document.getElementsByClassName("wPPcjs")[0].classList.remove("dark_uMWPUV");
				/*Enable : <Body> scroll->*/document.getElementsByClassName("overflowjs")[0].classList.remove("mobile_hide_scroll_Y_css");
				avatarBgColor = 1;
			}
		...
		document.addEventListener('click', function(event){
			...
			//Remove color from 3 buttoms
				...
				//Remove DARK color from 3 buttoms
					document.getElementsByClassName("wPPcjs")[0].classList.remove("dark_uMWPUV");
					document.getElementsByClassName("wPPcjs")[1].classList.remove("dark_uMWPUV");
			//Update
...
{//	publish-the-article.js
		...
		function publishTheArticle(){
			//Defined var from add new article
				let title         = document.getElementsByClassName("aitc")[0].value,
					...
					thumbnail_img = document.getElementsByClassName("thumbnail_image_js")[0],
					thumbnail_des = document.getElementsByClassName("thumbnail_description_js")[0].value,
					...
					//Check if text editor is NOT empty
							...
							//Check if text is enough
									...
									//Check if thumbnail is NOT uploaded
										if(thumbnail_img.files && thumbnail_img.files.length == 0){
											//ERROR
												document.getElementsByClassName("thumbnailErrorjs")[0].classList.remove("none");
												document.getElementsByClassName("thumbnailErrorjs")[0].innerHTML = "Thumbnail is required";
												document.getElementsByClassName("upload_thumbnail_input_js")[0].classList.add("redisbs");
												setTimeout(function(){
													removeThumbnailError();
												},7000);
										}
									//Check if thumbnail is uploaded
										else if(thumbnail_img.files && thumbnail_img.files.length > 0){
											//Check if thumbnail description is empty
												if(thumbnail_des.length == 0){
													//ERROR
														document.getElementsByClassName("thumbnailDescriptionErrorjs")[0].classList.remove("none");
														document.getElementsByClassName("thumbnailDescriptionErrorjs")[0].innerHTML = field_is_required;
														document.getElementsByClassName("thumbnail_description_js")[0].classList.add("redisbs");
												}
											//Check if thumbnail description is NOT empty
												else if(thumbnail_des.length > 0){
													last_check_for_new_article(thumbnail_des);
												}
											//Error when tried to check .thumbnail_description_js
												else {
													//ERROR
														alert("Error when tried to check .thumbnail_description_js");
												}
										}
									//Error when tried to check .thumbnail_image_js
										else {
											//ERROR
												alert("Error when tried to check .thumbnail_image_js");
										}
									//Last verification for last fields
										function last_check_for_new_article(thumbnail_des){
													...
													//Check if category froom list is NOT empty
															...
															//Check if notifications is allowed
																if(notifications == true){
																	/*Send article to server->*/ sats(title, text, thumbnail_des, category, '1');
																}
															//Check if notifications is NOT allowed
																else if(notifications == false){
																	/*Send article to server->*/ sats(title, text, thumbnail_des, category, '0');
																}
														}
												}
										}
	...
	//Remove Thumbnail error
		function removeThumbnailError(){
			document.getElementsByClassName("thumbnailErrorjs")[0].classList.add("none");
			document.getElementsByClassName("thumbnailErrorjs")[0].innerHTML = "";
			document.getElementsByClassName("upload_thumbnail_input_js")[0].classList.remove("redisbs");
		}
	//Remove Thumbnail Description error
		function removeThumbnailDescriptionError(){
			document.getElementsByClassName("thumbnail_description_js")[0].classList.remove("redisbs");
			document.getElementsByClassName("thumbnailDescriptionErrorjs")[0].classList.add("none");
		}
	//Send article to server
		function sats(title, text, thumbnail_des, category, notifications){	
			/*
				AJAX SEND
					article_title
					article_text
					article_thumbnail_des
					article_category
			...
			//AJAX
				...
				request.onreadystatechange = function(){
					if(request.status == 200 && request.readyState == 4){
						...
						//2.2 : article_thumbnail_des is empty
							else if(response == "2.2"){
								alert("article_thumbnail_des is empty");
							}
						//3 : ERROR NR.3
							else if(response == "3"){
								alert("ERROR NR.3");
						...
						//9 : Redirect author to yourself new added article
								...
								/*Send (second query after sats) thumbnail to server->*/ ssqastts(responseJSON.id);
							}
					}
				}
				request.open("POST", "../src/path/dt/ss/index/nav/nav-add_article/publish-now/publish-the-article.php");
				request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
				request.send("article_title=" + title + "&" + "article_text=" + text + "&" + "article_thumbnail_des=" + thumbnail_des + "&" + "article_category=" + category + "&" + "article_notifications=" + notifications);
		}
	//Send (second query after sats) thumbnail to server
		function ssqastts(responseJSONid){
			// Получаем файл
		    	var thumbnailIMG = document.getElementsByClassName("thumbnail_image_js")[0].files[0]; 
		    //Check if image is selected
			    if (thumbnailIMG) {
			        // Создаем объект FormData и добавляем изображение и responseJSONid
				        const formDataIMG = new FormData();
				        formDataIMG.append('thumbnailIMG', thumbnailIMG, 'thumbnail_image.png');
				        formDataIMG.append('responseJSONid', responseJSONid);
				        ...
				        request.onreadystatechange = function(){
				            if(request.readyState == 4 && request.status == 200){
				                // Обработка успешного ответа
				                	let response = request.responseText;
				                //1. success_PTP
				                	if(response == "success_PTP"){
				                		//alert("All steps are complited successfull");
										window.location.replace("article.php?id=" + responseJSONid);
				                	}
				            }
				        }
				        request.open("POST", "../src/path/dt/ss/index/nav/nav-add_article/publish-now/thumbnail/publish_thumbnail.php");
				        request.send(formDataIMG);
			        // END
			    } else {
			    	//ERROR
	...
	function uMWPU(nr){
		//Appearance
			...
			//Dark Mode
					...
					document.getElementsByClassName("wPPcjs")[0].classList.toggle("dark_uMWPUV");
				}
...

{
	...
	function ofc(net){
		if(net == 'off'){
			...
			/*show : message->*/warning_alert('You are offline');
		} else if(net == 'onl'){
			//document.getElementsByClassName("ofjs")[0].classList.add("none");
			/*show : message->*/warning_alert('You are online', 'success');
		}
	}
}
...
{//	unauthorized.js
	function unauthorized(){
		if(localStorage.getItem("session") == "false"){
			...
			document.getElementsByClassName("overflowjs")[0].style.overflowY = "hidden";
			document.getElementsByClassName("uunr1js")[0].classList.remove("none");
		}
	}

	function cuunr1(){
		document.getElementsByClassName("overflowjs")[0].style.overflowY = "scroll";
		document.getElementsByClassName("uunr1js")[0].classList.add("none");
	}
			...
			//Remove style for all buttons/dates
				for(let it = 0; it <= 34; it++){
					...
					//Dark Mode
							...
							document.getElementsByClassName("ca9js")[it].style.background = "#262B31";
							document.getElementsByClassName("ca9js")[it].style.color      = "white";
							document.getElementsByClassName("ca9js")[it].classList.add("ca9hjs");
						}
				}
	...
	//3 : function : light_mode, dark_mode, auto. [0 : Light; 1 : Dark; 2 : Auto];
		...
		//Auto
			function set_app_radio_auto(){
				...
				/*Bug-fix: User Menu Add Focus for Dark Mode when site is loaded->*/document.getElementsByClassName("ajs")[0].classList.add("dark_appearance_color_nr_2");
...
{//	WARNING/ALERT
	function warning_alert(message, status){
		...
		//START : color
			if(status == "success"){//green
				document.getElementsByClassName("warning_background")[0].classList.add("alert_background_green");
				document.getElementsByClassName("warning_background")[0].classList.remove("alert_background_gold");
			} else {//gold
				document.getElementsByClassName("warning_background")[0].classList.remove("alert_background_green");
				document.getElementsByClassName("warning_background")[0].classList.add("alert_background_gold");
			}
		//END
...
{//	Reload site if cache was deleted
	(function(){
	    function checkLocalStorage(){
	        if (localStorage.getItem("menu") !== "0" && localStorage.getItem("menu") !== "1"){
...
{
	function feed_SLIDER_btn_right(){
		/* move-to-right*/document.getElementsByClassName("slider_width_js")[0].style.marginLeft = "-1001px";
		/* hide-button : right*/document.getElementsByClassName("slider_move_right_js")[0].classList.add("none");
		/* show-button : left*/document.getElementsByClassName("slider_move_left_js")[0].classList.remove("none");
	}

	function feed_SLIDER_btn_left(){
		/* move-to-left*/document.getElementsByClassName("slider_width_js")[0].style.marginLeft = "0px";
		/* hide-button : left*/document.getElementsByClassName("slider_move_left_js")[0].classList.add("none");
		/* show-button : right*/document.getElementsByClassName("slider_move_right_js")[0].classList.remove("none");
	}
}

//disable_NEW_dark_theme();
function _disable_NEW_dark_theme(){
	document.getElementsByClassName("APPEARANCE_iiasfj_js")[0].classList.remove("DARK_THEME_COLOR_2");
	document.getElementsByClassName("APPEARANCE_spwh_js")[0].classList.remove("DARK_THEME_COLOR_2");
	document.getElementsByClassName("APPEARANCE_spwh_js")[0].classList.remove("APPEARANCE_DARK_sid_js");
	document.getElementsByClassName("ccbwnr5js")[0].classList.remove("dark_appearance_color_menu");
	document.getElementsByClassName("btn_home_js")[0].classList.remove("APPEARANCE_DARK_btn_HOME_js");
	document.getElementsByClassName("btn_home_js")[0].classList.remove("APPEARANCE_DARK__HOVER__btn_sidebar_js");
	document.getElementsByClassName("topics_show_more_TITLE_js")[0].classList.remove("APPEARANCE_DARK_topics_show_more_js");
	document.getElementsByClassName("APPEARANCE_MOBILE_search_js")[0].classList.remove("APPEARANCE_MOBILE_DARK_search_css");
	document.getElementsByClassName("appearance_nr_1_dual_js")[0].classList.remove("dark_appearance_color_nr_1");
	document.getElementsByClassName("APPEARANCE_search_js")[0].classList.remove("APPEARANCE_DARK_search_js");
	document.getElementsByClassName("APPEARANCE_article_js")[0].classList.remove("DARK_THEME_COLOR_1");
	document.getElementsByClassName("APPEARANCE_aaaw_js")[0].classList.remove("APPEARANCE_DARK_article_shadow_for_text_js");
	document.getElementsByClassName("APPEARANCE_continue_reading_js")[0].classList.remove("APPEARANCE_DARK_wGTR_js");
	document.getElementsByClassName("APPEARANCE_ad_js")[0].classList.remove("DARK_THEME_COLOR_1");
	document.getElementsByClassName("APPEARANCE_best_authors_js")[0].classList.remove("APPEARANCE_DARK_best_authors_js");
	document.getElementsByClassName("APPEARANCE_read_now_js")[0].classList.remove("APPEARANCE_DARK_read_now_js");
	document.getElementsByClassName("APPEARANCE_read_now_TITLE_js")[0].classList.remove("APPEARANCE_DARK_read_now_js");
	//document.getElementsByClassName("APPEARANCE_add_new_article_js")[0].classList.remove("APPEARANCE_DARK_add_new_article_js");
	document.getElementsByClassName("APPEARANCE_ad_js")[1].classList.remove("DARK_THEME_COLOR_1");
	document.getElementsByClassName("APPEARANCE_ad_js")[2].classList.remove("DARK_THEME_COLOR_1");
	document.getElementsByClassName("APPEARANCE_filter_new_js")[0].classList.remove("APPEARANCE_filter_new_anytime_css");
	document.getElementsByClassName("APPEARANCE_filter_new_js")[0].classList.remove("btn1_dark_mode_js");
	document.getElementsByClassName("btn2js")[0].classList.remove("APPEARANCE_DARK_rffanr2_js");
	document.getElementsByClassName("id_btn_filter_by_nr_3_from_8_js")[0].classList.remove("APPEARANCE_filter_new_anytime_css");
	document.getElementsByClassName("id_btn_filter_by_nr_3_from_8_js")[0].classList.remove("btn3_dark_mode_js");
	document.getElementsByClassName("id_btn_filter_by_nr_4_from_8_js")[0].classList.remove("APPEARANCE_DARK_rffanr2_js");
	document.getElementsByClassName("id_btn_filter_by_nr_4_from_8_js")[0].classList.remove("id_btn_filter_dark_mode_css");
	document.getElementsByClassName("id_btn_filter_by_nr_5_from_8_js")[0].classList.remove("APPEARANCE_DARK_rffanr2_js");
	document.getElementsByClassName("id_btn_filter_by_nr_5_from_8_js")[0].classList.remove("id_btn_filter_dark_mode_css");
	document.getElementsByClassName("id_btn_filter_by_nr_6_from_8_js")[0].classList.remove("APPEARANCE_DARK_rffanr2_js");
	document.getElementsByClassName("id_btn_filter_by_nr_6_from_8_js")[0].classList.remove("id_btn_filter_dark_mode_css");
	document.getElementsByClassName("id_btn_filter_by_nr_7_from_8_js")[0].classList.remove("APPEARANCE_DARK_rffanr2_js");
	document.getElementsByClassName("id_btn_filter_by_nr_7_from_8_js")[0].classList.remove("id_btn_filter_dark_mode_css");
	document.getElementsByClassName("id_btn_filter_by_nr_8_from_8_js")[0].classList.remove("APPEARANCE_DARK_rffanr2_js");
	document.getElementsByClassName("id_btn_filter_by_nr_8_from_8_js")[0].classList.remove("id_btn_filter_dark_mode_css");
	document.getElementsByClassName("APPEARANCE_title_best_authors_js")[0].classList.remove("APPEARANCE_DARK_title_best_authors_js");
	document.getElementsByClassName("APPEARANCE_comment_js")[0].classList.remove("APPEARANCE_DARK_cls_js");//loop
	document.getElementsByClassName("APPEARANCE_comment_ICON_js")[0].classList.remove("APPEARANCE_DARK_comment_ICON_js");//loop
	document.getElementsByClassName("APPEARANCE_like_js")[0].classList.remove("APPEARANCE_DARK_cls_js");//loop
	document.getElementsByClassName("APPEARANCE_cwcpi3_js")[0].classList.remove("APPEARANCE_DARK_cwcpi3_js");//loop
	document.getElementsByClassName("APPEARANCE_share_js")[1].classList.remove("APPEARANCE_DARK_cls_js");//loop
	document.getElementsByClassName("APPEARANCE_cwcpi4_js")[0].classList.remove("APPEARANCE_DARK_cwcpi4_js");//loop
	document.getElementsByClassName("APPEARANCE_button_more_filter_js")[0].classList.remove("APPEARANCE_DARK_button_more_filter_js");
	document.getElementsByClassName("APPEARANCE_filter_icon_calendar_js")[0].classList.remove("APPEARANCE_DARK_filter_icon_calendar_js");
	document.getElementsByClassName("APPEARANCE_filter_calendar_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_icon_arrow_moreFilters_js")[0].classList.remove("APPEARANCE_DARK_icon_arrow_articleL_js");
	document.getElementsByClassName("APPEARANCE_choose_a_date_js")[0].classList.remove("APPEARANCE_DARK_choose_a_date_js");
	document.getElementsByClassName("APPEARANCE_calendar_title_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_FROM_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_UPTO_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_input_FROM_js")[0].classList.remove("APPEARANCE_calendar_input_FROM_UPTO_css");
	document.getElementsByClassName("APPEARANCE_calendar_input_UPTO_js")[0].classList.remove("APPEARANCE_calendar_input_FROM_UPTO_css");
	document.getElementsByClassName("APPEARANCE_calendar_move_left_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_name_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_move_right_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_nr_1F7_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_nr_2F7_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_nr_3F7_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_nr_4F7_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_nr_5F7_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_nr_6F7_js")[0].classList.remove("APPERANCE_DARK_text_css");
	document.getElementsByClassName("APPEARANCE_calendar_nr_7F7_js")[0].classList.remove("APPERANCE_DARK_text_css");
	//document.getElementsByClassName("APPEARANCE_SignIn_js")[0].classList.remove("APPEARANCE_DARK_SignIn_js");
	//document.getElementsByClassName("APPEARANCE_puwcpi_js")[0].classList.remove("APPEARANCE_DARK_puwcpi_js");
	//document.getElementsByClassName("APPEARANCE_connectFormUSERNAMEorEMAIL_js")[0].classList.remove("APPEARANCE_DARK_connectFormUSERNAMEorEMAIL_js");
	//document.getElementsByClassName("APPEARANCE_connectFormPASSWORD_js")[0].classList.remove("APPEARANCE_DARK_connectFormPASSWORD_js");
	//document.getElementsByClassName("APPEARANCE_recoverAccess_js")[0].classList.remove("APPEARANCE_DARK_recoverAccess_js");
	//document.getElementsByClassName("APPEARANCE_createAccount_js")[0].classList.remove("APPEARANCE_DARK_createAccount_js");
	//document.getElementsByClassName("APPEARANCE_forgot_password_js")[0].classList.remove("APPEARANCE_DARK_forgot_password_js");
	//document.getElementsByClassName("APPEARANCE_restoringAccessFP_js")[0].classList.remove("APPEARANCE_DARK_restoringAccessFP_js");
	//document.getElementsByClassName("APPEARANCE_registration_js")[0].classList.remove("APPEARANCE_DARK_registration_js");
	//document.getElementsByClassName("APPEARANCE_region_js")[0].classList.remove("APPEARANCE_DARK_region_js");
	//document.getElementsByClassName("APPEARANCE_languages_js")[0].classList.remove("APPEARANCE_DARK_languages_js");
	//document.getElementsByClassName("APPEARANCE_iconRegion_js")[0].classList.remove("APPEARANCE_DARK_iconRegion_js");
	//document.getElementsByClassName("APPEARANCE_item_region_ENG_js")[0].classList.remove("APPEARANCE_DARK_item_region_ENG_js");
	//document.getElementsByClassName("APPEARANCE_item_region_ENG_js")[0].classList.remove("aIGreen_dark_mode_js");
	//document.getElementsByClassName("APPEARANCE_item_region_SPA_js")[0].classList.remove("APPEARANCE_DARK_item_region_SPA_js");
	//document.getElementsByClassName("APPEARANCE_item_region_FRE_js")[0].classList.remove("APPEARANCE_DARK_item_region_FRE_js");
	//document.getElementsByClassName("APPEARANCE_item_region_TUR_js")[0].classList.remove("APPEARANCE_DARK_item_region_TUR_js");
	//document.getElementsByClassName("APPEARANCE_item_region_POR_js")[0].classList.remove("APPEARANCE_DARK_item_region_POR_js");
	//document.getElementsByClassName("APPEARANCE_item_region_ROM_js")[0].classList.remove("APPEARANCE_DARK_item_region_ROM_js");
	//document.getElementsByClassName("APPEARANCE_item_region_DUT_js")[0].classList.remove("APPEARANCE_DARK_item_region_DUT_js");
	//document.getElementsByClassName("APPEARANCE_item_region_POL_js")[0].classList.remove("APPEARANCE_DARK_item_region_POL_js");
	//document.getElementsByClassName("APPEARANCE_item_region_UKR_js")[0].classList.remove("APPEARANCE_DARK_item_region_UKR_js");
	document.getElementsByClassName("APPEARANCE_articleLButton_js")[0].classList.remove("APPEARANCE_DARK_articleLButton_js");
	document.getElementsByClassName("APPEARANCE_dC1_js")[0].classList.remove("APPEARANCE_DARK_dc123_js");
	document.getElementsByClassName("APPEARANCE_dC2_js")[0].classList.remove("APPEARANCE_DARK_dc123_js");
	document.getElementsByClassName("APPEARANCE_dC3_js")[0].classList.remove("APPEARANCE_DARK_dc123_js");
	document.getElementsByClassName("APPEARANCE_logoSVG_js")[0].classList.remove("APPEARANCE_DARK_logoSVG");
	document.getElementsByClassName("btn_home_ICON_js")[0].classList.remove("APPEARANCE_DARK_btn_home_ICON_js");
	document.getElementsByClassName("btn_home_TITLE_js")[0].classList.remove("APPEARANCE_DARK_btn_home_TITLE_js");
	document.getElementsByClassName("btn_likes_ICON_js")[0].classList.remove("APPEARANCE_DARK_btn_likes_ICON_js");
	document.getElementsByClassName("btn_likes_TITLE_js")[0].classList.remove("APPEARANCE_DARK_btn_likes_TITLE_js");
	document.getElementsByClassName("btn_history_ICON_js")[0].classList.remove("APPEARANCE_DARK_btn_history_ICON_js");
	document.getElementsByClassName("btn_history_TITLE_js")[0].classList.remove("APPEARANCE_DARK_btn_history_TITLE_js");
	document.getElementsByClassName("btn_topics_TITLE_js")[0].classList.remove("APPEARANCE_DARK_btn_topics_TITLE_js");
	document.getElementsByClassName("btn_resources_TITLE_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_TITLE_js");
	document.getElementsByClassName("APPEARANCE_btn_ABOUT_js")[0].classList.remove("APPEARANCE_DARK_btn_ABOUT_EL_ICON_js");
	document.getElementsByClassName("APPEARANCE_btn_TERMS_js")[0].classList.remove("APPEARANCE_DARK_btn_TERMS_EL_ICON_js");
	document.getElementsByClassName("APPEARANCE_btn_COPYRIGHT_js")[0].classList.remove("APPEARANCE_DARK_btn_COPYRIGHT_EL_ICON_js");
	document.getElementsByClassName("APPEARANCE_btn_PRIVACY_js")[0].classList.remove("APPEARANCE_DARK_btn_PRIVACY_EL_ICON_js");
	document.getElementsByClassName("APPEARANCE_btn_ADVERTISEMENT_js")[0].classList.remove("APPEARANCE_DARK_btn_ADVERTISEMENT_EL_ICON_js");
	document.getElementsByClassName("APPEARANCE_btn_HELP_js")[0].classList.remove("APPEARANCE_DARK_btn_HELP_EL_ICON_js");
	document.getElementsByClassName("APPEARANCE_btn_FEEDBACK_js")[0].classList.remove("APPEARANCE_DARK_btn_FEEDBACK_EL_ICON_js");
	document.getElementsByClassName("APPEARANCE_btn_TITLE_ABOUT_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_EL_TITLE_js");
	document.getElementsByClassName("APPEARANCE_btn_TITLE_TERMS_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_EL_TITLE_js");
	document.getElementsByClassName("APPEARANCE_btn_TITLE_COPYRIGHT_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_EL_TITLE_js");
	document.getElementsByClassName("APPEARANCE_btn_TITLE_PRIVACY_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_EL_TITLE_js");
	document.getElementsByClassName("APPEARANCE_btn_TITLE_ADVERTISEMENT_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_EL_TITLE_js");
	document.getElementsByClassName("APPEARANCE_btn_TITLE_HELP_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_EL_TITLE_js");
	document.getElementsByClassName("APPEARANCE_btn_TITLE_FEEDBACK_js")[0].classList.remove("APPEARANCE_DARK_btn_resources_EL_TITLE_js");
	document.getElementsByClassName("APPEARANCE_footer_text_js")[0].classList.remove("APPEARANCE_DARK_footer_text_js");
	document.getElementsByClassName("APPEARANCE_footer_c_js")[0].classList.remove("APPEARANCE_DARK_footer_c_js");
	document.getElementsByClassName("APPEARANCE_date_js")[0].classList.remove("APPEARANCE_DARK_date_js");//loop
}

function neeewww_dark_theme(){
	document.getElementsByClassName("innerHTML_sorted_bottoms_js")[1].innerHTML = "";
	document.getElementsByClassName("innerHTML_sorted_bottoms_js")[2].innerHTML = "";
	MenuAuto();
}
		...
		//START : Query
			...
			request.onreadystatechange = function(){
				if(request.readyState == 4 && request.status == 200){
					//If complain/message was successully send
						if(request.responseText == "1"){
							alert("message was successully send");
						}
				}
			}
			request.open('POST', '../src/path/dt/ss/index/sidebar/sidebar-left/sidebar-home/sidebar-home-articles-make_star/make_star.php');
			request.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
			request.send("id=" + String(id).slice(0, -3));
		//END
	}
...
/*START : Recommend/Right window : Recommended*/
	function makeRecommended(id){
		//START : Query
			let request = new XMLHttpRequest();
			request.onreadystatechange = function(){
				if(request.readyState == 4 && request.status == 200){
			...
			request.open('POST', '../src/path/dt/ss/index/sidebar/sidebar-left/sidebar-home/sidebar-home-articles-make_recommended/make_recommended.php');
			request.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
			request.send("id=" + String(id).slice(0, -3));
		//END
	}
...

{
	//0 : Update banner
		function update_AD(id){
			//First banner
				if(id == 'A1'){
					send_new_banner_URL_ts(0);
				}
			//Second banner
				else if(id == 'A2'){
					send_new_banner_URL_ts(1);
				}
			//Third banner
				else if(id == 'A3'){
					send_new_banner_URL_ts(2);
				}

		}
	//1 : Send new banner URL to server
		function send_new_banner_URL_ts(id){
			
		}
	//2 : Send new banner to server
		function send_new_banner_ts(id){
			//Получаем файл
		    	var thumbnailIMG = document.getElementsByClassName("thumbnail_BANNER_js")[id].files[0]; 
		    //Check if image is selected
			    if (thumbnailIMG) {
			        // Создаем объект FormData и добавляем изображение и responseJSONid
				        const formDataIMG = new FormData();
			        ...
			        // START: Здесь можно выполнить отправку изображения на сервер через Ajax
				        ...
				        request.open("POST", "../src/path/dt/ss/index/sidebar/sidebar-right/ad/UPDATE_2_IMG_AD.php");
				        request.send(formDataIMG);
			        // END
			    } else {
			    	//ERROR
